...
var getResource = {
  preStartUrl: 'https://m.manhuadui.com',       //查看某一话的前缀url
  imgPreUrl: 'https://img01.eshanyao.com/',       //某些图片需要加此前缀
  ...
  getInfoList: function(){
    ...
    return new Promise(function(resolve){
      that.getUrlDom(that.listUrl).then((obj)=>{
        //查询集数 和 每一集的链接
        var list = obj['$']('.chapter-warp').find('ul li');
        let jishuList = [];
        for(let i=0;i<list.length;i++){
          let info = {
            title: obj['$'](list).eq(i).find('span').eq(0).text(),
    ...
    return new Promise(function(resolve){
      ...
      that.getUrlDom(url).then((obj)=>{
          if(obj['domStr']){
              ...
              let images = that.dealImages(str); //每一集的所有图片的后缀集合
  ...
  /**
   * 处理加密的图片
   * chapterImages  加密的图片字符串
   * 返回结果为图片数组，[]
   */
  dealImages: function(chapterImages){
